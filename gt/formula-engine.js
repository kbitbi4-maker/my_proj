...
class FormulaEngine {
    ...
    evaluateAST(ast, sheetName) {
        switch (ast.type) {
            ...
            case 'cellRef': return this.getCellValue(sheetName, ast.col, ast.row);
            case 'sheetRef': return this.getCellValue(ast.sheet, this.parser.parseCellRef(ast.cell).col, this.parser.parseCellRef(ast.cell).row);
            case 'range': return this.getRangeValues(sheetName, ast.from, ast.to);
            case 'sheetRange': return this.getRangeValues(ast.sheet, ast.from, ast.to);
            case 'binary': return this.evaluateBinary(ast, sheetName);
            case 'function': return this.evaluateFunction(ast, sheetName);
            default: return '#ERROR: Неизвестный тип AST';
        }
    }
    ...
    evaluateFunction(ast, sheetName) {
        ...
        switch (funcName) {
            case 'SUM': return flatArgs.reduce((sum, val) => sum + (parseFloat(val) || 0), 0);
            case 'AVERAGE': {
                const numbers = flatArgs.filter(v => !isNaN(parseFloat(v)));
                return numbers.length > 0 ? numbers.reduce((sum, v) => sum + v, 0) / numbers.length : 0;
            }
            case 'COUNT': return flatArgs.filter(v => v !== '' && v !== null && v !== undefined).length;
            case 'MAX': {
            ...
            case 'ABS': return Math.abs(parseFloat(args[0]) || 0);
            case 'POWER': return Math.pow(parseFloat(args[0]) || 0, parseFloat(args[1]) || 0);
            case 'SQRT': {
                const sqrtVal = parseFloat(args[0]) || 0;
                return sqrtVal >= 0 ? Math.sqrt(sqrtVal) : '#ERROR: Отрицательное число';
            }
            default: return '#ERROR: Неизвестная функция ' + funcName;
        }
    }
        ...
        for (const sheetName in this.data) {
            ...
            for (let r = 0; r < rows.length; r++) {
                ...
                for (let c = 0; c < rows[r].length; c++) {
                    const value = rows[r][c];
                    formulas[sheetName][r][c] = this.parser.isFormula(value) ? value : '';
                }
            }
        }
        return formulas;
