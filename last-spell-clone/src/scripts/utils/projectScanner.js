export class ProjectScanner{
    constructor(){
        // РЕАЛЬНАЯ структура вашего проекта (обновите под свои файлы)
        this.files = {
            'index.html': true,
            'README.md': true,
            '.gitignore': true,
    ...
    // Метод для обновления статуса файла
    setFileStatus(filePath, exists){
        this.files[filePath] = exists;
    }
    
    // Метод для добавления нового файла в структуру
    addFile(filePath, exists = false){
        this.files[filePath] = exists;
    }
