...
export const ADDITION_RULES = [
    {
        ...
        config: (ctx) => ({
            ...
            leftTruck: { label: String(ctx.math.num1), color: "#0284c7", isOrange: false, hundreds: ctx.mode === 'hundreds' ? Math.floor(ctx.math.num1 / 100) : 0, tens: ctx.math.tens1, ones: ctx.math.ones1, colorBorrow: 0 },
            rightTruck: { label: String(ctx.math.num2), color: "#ea580c", isOrange: true, hundreds: ctx.mode === 'hundreds' ? Math.floor(ctx.math.num2 / 100) : 0, tens: ctx.math.tens2, ones: ctx.math.ones2, colorBorrow: 0 },
    ...
    },
    {
        ...
        config: (ctx) => {
            ...
            return {
                ...
                leftTruck: { label: String(uL), color: "#22c55e", isOrange: false, style: glow, hundreds: lH, mixedHundreds: lM, tens: Math.floor(uL / 10) % 10, ones: uL % 10, colorBorrow: isLeftRound ? d.leftBorrowCount : 0 },
                rightTruck: { label: String(uR), color: "#ea580c", isOrange: true, style: ctx.simCorrect ? 'filter:drop-shadow(0 0 6px #facc15);' : '', hundreds: rH, mixedHundreds: rM, tens: Math.floor(uR / 10) % 10, ones: uR % 10, colorBorrow: isRightRound ? d.rightBorrowCount : 0 },
    ...
    },
    {
        ...
        config: (ctx) => {
            ...
            return {
                ...
                leftTruck: { tens: d.tens1, ones: d.ones1, isLeftRound: uL > d.num1, isRightRound: uL <= d.num1, colorBorrow: uL > d.num1 ? d.leftBorrowCount : d.rightBorrowCount },
                rightTruck: { tens: d.tens2, ones: d.ones2 },
