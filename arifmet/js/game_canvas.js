// version: v2.0  
import { selectExample } from './view\_dispatcher.js'; 
...
// Если simHTML пустой (как при прямом вводе в умножении), мы аккуратно скрываем обертку  
const simWrapper = line.querySelector('.sim-block-wrapper');  
const finWrapper = line.querySelector('.fin-block-wrapper'); 

if (simWrapper) simWrapper.innerHTML = simHTML || '';  
if (finWrapper) finWrapper.innerHTML = finHTML || '';  
}); 
...
clearHistory() {  
if (examplesList) {  
examplesList.innerHTML = 'Выберите режим в меню для начала игры';  
}  
}, 
...
return `<div class="${animationClass}" style="display:flex;flex-direction:column;align-items:center;justify-content:center;"><span style="font-size:46px;line-height:1;filter:drop-shadow(0 2px 2px rgba(0,0,0,0.1));">${emoji}</span>${labelHTML}${subtitle ?` ${subtitle} `: ''}</div>`;  
}, 
