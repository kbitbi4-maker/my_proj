function renderSubtractionVisual(num1, num2, currentInput) {
    ...
    let html = '';
    if (!hasPressedEqual) { // ФАЗА 1: СТАРТ. Л стоит слева от синего груза, у П сзади белые кубики
...
function generateSubCargoHTML(tens, ones, added, subtracted) {
    ...
    for (let i = 0; i < fullColumns; i++) { // Полные столбики десятков
        html += `<div class="crystal-column">`;
        for (let j = 0; j < 10; j++) {
            globalCounter++; // Если кубик потерян при убавлении, он становится пустым белым с черным контуром
            let extraClass = (globalCounter <= activeCubes) ? 'borrow-blue' : '';
            let inlineStyle = (globalCounter > activeCubes) ? 'style="border:1px solid #000; background:#fff; box-shadow:none;"' : '';
            html += `<div class="crystal-item ${extraClass}" ${inlineStyle}></div>`;
        }
        html += `</div>`;
    }
    if (remOnes > 0) { // ИСПРАВЛЕНО: 10 фиксированных ячеек для неполного столбика единиц (снизу вверх)
        ...
        for (let j = 1; j <= 10; j++) {
            if (j <= remOnes) {
                ...
                let inlineStyle = (globalCounter > activeCubes) ? 'style="border:1px solid #000; background:#fff; box-shadow:none;"' : '';
                html += `<div class="crystal-item ${extraClass}" ${inlineStyle}></div>`;
            } else { // Верхняя пустая часть столбика (до потолка) прозрачна
...
function generateSubEmptyCubesHTML(emptyCount, addedOrangeCount) {
    let html = '<div class="crystal-column">'; // ИСПРАВЛЕНО: Сплошной черный контур и белый цвет внутри
    for (let i = 0; i < emptyCount; i++) html += `<div class="crystal-item" style="border: 1px solid #000; background: #fff; box-shadow: none;"></div>`;
    for (let i = 0; i < addedOrangeCount; i++) html += `<div class="crystal-item borrow-orange"></div>`; // Оранжевые кубики упрощения
    return html + '</div>';
}
